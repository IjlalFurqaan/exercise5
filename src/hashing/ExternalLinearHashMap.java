...
public class ExternalLinearHashMap<K, V> implements Iterable<MapEntry<K, V>> {
    ...
    private class HashBucket implements Iterable<MapEntry<K, V>> {
        ...
        ProbeResult probe(K key) {
            ...
            while (true) {
                // Search for key in current block
                ...
                // If no overflow block exists, key was not found
        ...
        V insert(K key, V value) {
            ...
            if (res.entry != null) {
                ...
                res.block.clearElements();
                for (MapEntry<K, V> entry : res.block) {
                    if (entry.getKey().equals(key)) {
            ...
            if (res.block.getSize() < elementsPerBlock) {
                // Space available in current block
                res.block.add(new MapEntry<>(key, value));
                res.container.update(res.blockId, res.block);
            } else {
                // Need new overflow block
                long newId = newOverflowId();
                HashBlock<K, V> newBlock = new HashBlock<>();
                newBlock.add(new MapEntry<>(key, value));
                res.block.setOverflowId(newId);
                res.container.update(res.blockId, res.block);
                secondary.update(newId, newBlock);
            }
            return null;
        }
    ...
    private int realHashIndex(K key) {
        int hash = hashIndex(key, level);
        if (hash < expansionPointer) {
            // This bucket has already been split, use next level hash
            hash = hashIndex(key, level + 1);
        }
        return hash;
    }
    /**
